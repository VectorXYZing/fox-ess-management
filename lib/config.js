...
(function applyEnvOverrides() {
  ...
  if (e.LATITUDE || e.LONGITUDE || e.SYSTEM_KW) {
    ...
    if (e.LATITUDE) {
      const v = parseFloat(e.LATITUDE);
      if (Number.isFinite(v)) config.solar.latitude = v;
      else console.warn(`config: invalid LATITUDE env var "${e.LATITUDE}" — ignoring`);
    }
    if (e.LONGITUDE) {
      const v = parseFloat(e.LONGITUDE);
      if (Number.isFinite(v)) config.solar.longitude = v;
      else console.warn(`config: invalid LONGITUDE env var "${e.LONGITUDE}" — ignoring`);
    }
    if (e.SYSTEM_KW) {
      const v = parseFloat(e.SYSTEM_KW);
      if (Number.isFinite(v)) config.solar.systemKw = v;
      else console.warn(`config: invalid SYSTEM_KW env var "${e.SYSTEM_KW}" — ignoring`);
    }
  }
  if (e.BATTERY_KWH) {
    const v = parseFloat(e.BATTERY_KWH);
    if (Number.isFinite(v)) {
      config.battery = config.battery || {};
      config.battery.capacityKwh = v;
    } else {
      console.warn(`config: invalid BATTERY_KWH env var "${e.BATTERY_KWH}" — ignoring`);
    }
  }
})();
...
function saveConfig(next) {
  ...
  for (const cb of _onSaveCallbacks) { try { cb(); } catch (e) { console.warn('config: onSaved callback error:', e.message); } }
}
