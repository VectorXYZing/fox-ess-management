...
// ---------------------------------------------------------------------------
const MAX_SOC_FILE_BYTES = 10 * 1024 * 1024; // 10 MB

function readSocSnapshots() {
  if (!fs.existsSync(SOC_HISTORY_PATH)) return [];
  const stat = fs.statSync(SOC_HISTORY_PATH);
  if (stat.size > MAX_SOC_FILE_BYTES) {
    console.warn(`soc-history: file is ${stat.size} bytes — exceeds limit, skipping read`);
    return [];
  }
  const out = [];
  const lines = fs.readFileSync(SOC_HISTORY_PATH, 'utf8').split('\n');
  lines.forEach((line, i) => {
    if (!line.trim()) return;
    try { out.push(JSON.parse(line)); }
    catch { console.warn(`soc-history: bad JSON on line ${i + 1}, skipping`); }
  });
  return out;
}
