...
async function getCurrentSoC() {
  await throttleFox();
  const r      = await foxRequest('/op/v1/device/real/query', {
    sn: config.deviceSN, deviceSN: config.deviceSN, sns: [config.deviceSN], variables: ['SoC'],
  });
  const parsed = JSON.parse(r.body);
...
function buildNarrative(ctx) {
  ...
  if (c.action === 'solarShort') {
    ...
    if (c.pastTopUpWindow && c.cfg?.allowGridCharging) {
      return `Battery's ${sh} kWh short of a full dump and we're past the midday top-up window (${c.cfg.topUpSearchStart}–${c.cfg.topUpSearchEnd}). `
        + `Too late in the day to grid-charge profitably. The dump will run with what you have (~${dumpKwh} kWh, $${dumpRev}). `
        + `Tomorrow's plan will refresh by 10:30 am.`;
    }
    if (c.aemoUnavailable && c.cfg?.allowGridCharging) {
...
async function computeRecommendation() {
  ...
  let aemoUnavailable  = false;       // true only when AEMO genuinely failed
  let pastTopUpWindow  = false;       // true when AEMO ok but we're past 15:00
  ...
  let subline, note;
  if (aemoUnavailable) {
    subline = 'AEMO forecast unavailable.';
    note    = forecastNote || 'AEMO forecast unavailable; cannot assess arbitrage.';
  } else if (pastTopUpWindow) {
    subline = `Past today's top-up window (${cfg.topUpSearchStart}–${cfg.topUpSearchEnd}).`;
    note    = forecastNote;
  } else {
    subline = `Cheapest midday retail ${cheapestRetailCents}¢ vs ${round1(breakEvenImportCents)}¢ break-even.`;
    note    = `Cheapest midday retail ≈ ${cheapestRetailCents} c/kWh ≥ break-even ${round1(breakEvenImportCents)} c/kWh.`;
  }
  return {
    ...
    subline, note,
    narrative : buildNarrative({ action: 'solarShort', cfg, currentSoCPct, solarRemainingKwh, expectedDumpKwh, expectedDumpRevenueDollars, shortfallKwh, cheapestRetailCents, breakEvenImportCents, pastTopUpWindow, aemoUnavailable }),
    aemoUnavailable,
    pastTopUpWindow,
  };
}
...
// ---------------------------------------------------------------------------
// Failure-result short-cache: if AEMO forecast was unavailable last run,
// don't make the user wait 15 min for the next retry — re-compute after 60 s.
const AEMO_RETRY_MS = 60 * 1000;

// A cached result older than one tick interval is considered stale (a tick
// probably failed silently). We still serve it rather than blocking, but flag it.
const STALE_THRESHOLD_MS = TICK_MS + 60_000;

async function getRecommendation() {
  const cached = recommendationCache.get();
  if (cached) {
    const ageMs = Date.now() - recommendationCache.at;
    const isStaleFailure = cached.aemoUnavailable && ageMs > AEMO_RETRY_MS;
    if (!isStaleFailure) {
      return { ...cached, isStale: ageMs > STALE_THRESHOLD_MS, cacheAgeMs: ageMs };
    }
  }
  const rec = await computeRecommendation();
  recommendationCache.set(rec);
  return { ...rec, isStale: false, cacheAgeMs: 0 };
}
