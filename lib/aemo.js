...
async function fetchForecastForRegion(region) {
  ...
  // Don't cache empty results — caching nothing for 5 min blocks legitimate
  // retries when nemweb hiccups or returns a partial file.
  if (Object.keys(byRegion).length === 0 || !byRegion[region] || byRegion[region].length === 0) {
    return byRegion[region] || [];
  }
